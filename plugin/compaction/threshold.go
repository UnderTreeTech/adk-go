...
func (s *thresholdStrategy) Compact(ctx agent.CallbackContext, req *model.LLMRequest) error {
	...
	for attempt := range s.maxCompactionAttempts {
		log.Info(ctx, "ContextGuard [threshold]: threshold exceeded, summarizing",
			log.String("agent", ctx.AgentName()),
			log.String("session", ctx.SessionID()),
			log.Int("attempt", attempt+1),
			log.Int("tokens", totalTokens),
			log.Int("threshold", threshold),
			log.Int("context_window", contextWindow),
			log.Int("buffer", buffer),
			log.Int("max_summary_words", int(float64(buffer)*0.50*0.75)),
		)
		...
		if err != nil {
			log.Warn(ctx, "ContextGuard [threshold]: summarization failed, using fallback",
				log.String("agent", ctx.AgentName()),
				log.String("session", ctx.SessionID()),
		...
		log.Info(ctx, "ContextGuard [threshold]: compaction pass completed",
			log.String("agent", ctx.AgentName()),
			log.String("session", ctx.SessionID()),
			log.Int("attempt", attempt+1),
			log.Int("old_messages", len(req.Contents)),
		...
		if attempt < s.maxCompactionAttempts-1 {
			log.Warn(ctx, "ContextGuard [threshold]: still above threshold after compaction, retrying",
				log.String("agent", ctx.AgentName()),
				log.Int("attempt", attempt+1),
